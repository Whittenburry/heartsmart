...
import { ExtractedRecipe } from './extract'
...
const schemaFields = {
  ...
  properties: {
    ...
    optimizedIngredients: { type: Type.ARRAY, items: { type: Type.STRING } },
    optimizedInstructions: { type: Type.ARRAY, items: { type: Type.STRING } },
    substitutions: {
      type: Type.ARRAY,
      items: {
        type: Type.OBJECT,
    ...
    healthRationale: { type: Type.ARRAY, items: { type: Type.STRING } },
    cautions: { type: Type.ARRAY, items: { type: Type.STRING } },
    estimatedNutritionNotes: { type: Type.ARRAY, items: { type: Type.STRING } }
  },
  required: [
...
const optimizeRoutes: FastifyPluginAsync = async (server: FastifyInstance) => {
  server.post<{ Body: { recipe: ExtractedRecipe, healthGoal: string } }>('/api/optimize-recipe', async (request, reply) => {
